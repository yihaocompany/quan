...
(function($) {
	 ...
	 * @type {object}
	 */
	var YihaoCMS = window.Sent;

	/* 基础对象检测 */
	YihaoCMS || $.error("YihaoCMS基础配置没有正确加载！");
	 ...
	 */
	YihaoCMS.parse_url = function(url) {
	...
	YihaoCMS.parse_str = function(str) {
		var value = str.split("&"),
			vars = {},
			param;
		for (val in value) {
	...
	YihaoCMS.parse_name = function(name, type) {
		if (type) {
			/* 下划线转驼峰 */
	...
	YihaoCMS.U = function(url, vars, suffix) {
		...
		if (0 === url.indexOf("/")) { //路由模式
			...
			path[1] || $.error("YihaoCMS.U(" + url + ")没有指定控制器");
	...
	/* 设置表单的值 */
	YihaoCMS.setValue = function(name, value) {
